...
**The mount at `/var/lib/postgresql` needs an image that knows how to move the cluster into the
versioned folder**, which is every repository image from the release that added
`psql_relocate.sh` on. Deploying the new mount with an older image is the one combination that
loses data quietly: the data directory is simply absent, `docker-entrypoint.sh` initializes an
empty cluster beside the real one, and the job comes up healthy serving nothing. Raise the `FROM`
of a derived image in the same change that moves its mount.
...
```sh
PGPASSWORD=... docker run --rm -e PGPASSWORD \
...
2. Point the deployment at this image. For a deployment that derives from the repository image,
   ...
   the derived image, its configuration and its scripts, stays as it is. **The derived Dockerfile
   must not declare an `ENTRYPOINT` of its own**, which some of them do: the migration is wired
   through the one this image sets, so an image that redeclares it inherits the new base and not
   the new entrypoint, and nothing is migrated. Point it at `./psql_upgrade_init.sh` for the
   window, or drop the line. This fails safely — the server refuses to start on data of a version
   it cannot read — but the message names the image already deployed, which is a confusing place
...
```
 ...
 7. run the first pass of the statistics rebuild, which pg_upgrade does not carry over:
        vacuumdb --all --analyze-in-stages -U <admin>
    the server plans every query on no statistics until it finishes
 8. check the data, then let traffic back in
```

Before step 5, raise or disable the job's health check grace and deployment timeout: the
migration happens inside the container start, and an orchestrator that kills it while
pg_upgrade is linking leaves the one state that needs the snapshot. Restore them afterwards.
...
- **Rebuild the replica from scratch, on a volume mounted at `/var/lib/postgresql`.**
  `pg_upgrade` gives the cluster a new system identifier, so the old replica can never follow it
  again. A fresh volume is enough: `psql_replica_init.sh` finds no `replication_configured.lock`
  and takes a full `pg_basebackup`. Move its mount to `/var/lib/postgresql` in the same change
  that gives it the fresh volume, and not before: the replica image has no relocation step, so
  remounting a volume that already holds a cluster makes the data directory look empty, and the
  replica wipes it and clones the primary again while the old copy stays at the volume root.
...
- **Move the deployment back to the repository image** on the next routine release. The migration
  is recorded on the volume and does not run again, and the upgrade image carries binaries the
  deployment no longer needs. Nothing it does is silent: the configuration is read and refused,
  never rewritten. That check runs before a migration, not on every start — once the volume
  records the migration as done the image hands over immediately, so staying on it does not
  keep re-validating anything.
...
**The repository images stay on Debian bookworm, and a major upgrade must not change that.**
Collation-aware indexes carry the sort order of the glibc they were built with, and `pg_upgrade`
carries them over untouched. Crossing a glibc version therefore means the server reports a
collation version mismatch and text indexes can silently return incomplete results — a row that
is there and is not found — until every one of them is rebuilt. That is not a cheap way out:
`REINDEX` takes a lock that blocks reads and writes, so on a large database it is days with the
database unavailable, and `REINDEX CONCURRENTLY` needs room for a second copy of every index and
leaves invalid ones behind when it fails.

The plain `postgres:18` tag is trixie (glibc 2.41) while 14 to 17 are bookworm (glibc 2.36),
which is why `repository/Dockerfile7` names `postgres:18.1-bookworm`. Keep the suffix on every
version this repository builds, and look again before adopting a major whose bookworm variant
upstream has stopped publishing: that is the release where this stops being free, and the
reindex becomes part of the window rather than something the image avoids.
...
the job: `copy` migrates into a second copy of the data, leaving the old cluster startable, at
the cost of room on the volume and of a migration that takes as long as the data is large.
`auto` picks `copy` when the volume has the room. The image ships `link`.

**`copy` only leaves a rollback behind together with `UPGRADE_KEEP_OLD_DATA=true`.** On its own
it deletes the old cluster once the migration is recorded, which is the default and is announced
in the log after the window has already been spent. And the copy it keeps does not come back by
redeploying the repository image: it sits at `<volume>/.pg_upgrade_old_<version>`, where nothing
looks for it, while the migrated cluster is what the data directory now holds. Bringing it back
is a manual rename on the volume, with the job stopped.
