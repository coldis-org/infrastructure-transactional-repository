...
do
	...
	PGPASSWORD=${POSTGRES_PASSWORD} psql -c "ALTER DEFAULT PRIVILEGES IN SCHEMA \"public\" REVOKE ALL ON TABLES FROM \"${READ_USER}\";" -U ${POSTGRES_USER}
	PGPASSWORD=${POSTGRES_PASSWORD} psql -c "ALTER DEFAULT PRIVILEGES IN SCHEMA \"public\" REVOKE ALL ON SEQUENCES FROM \"${READ_USER}\";" -U ${POSTGRES_USER}
	PGPASSWORD=${POSTGRES_PASSWORD} psql -c "ALTER DEFAULT PRIVILEGES IN SCHEMA \"public\" REVOKE ALL ON FUNCTIONS FROM \"${READ_USER}\";" -U ${POSTGRES_USER}
	PGPASSWORD=${POSTGRES_PASSWORD} psql -c "REVOKE ALL ON SCHEMA \"public\" FROM \"${READ_USER}\";" -U ${POSTGRES_USER}
	PGPASSWORD=${POSTGRES_PASSWORD} psql -c "REVOKE ALL ON DATABASE \"${DATABASE_NAME}\" FROM \"${READ_USER}\";" -U ${POSTGRES_USER}
	PGPASSWORD=${USER_PASSWORD} psql -c "REVOKE ALL ON SCHEMA \"public\" FROM \"${READ_USER}\";" -U ${USER_NAME}
	PGPASSWORD=${USER_PASSWORD} psql -c "REVOKE ALL ON DATABASE \"${DATABASE_NAME}\" FROM \"${READ_USER}\";" -U ${USER_NAME}
	PGPASSWORD=${USER_PASSWORD} psql -c "REVOKE ALL ON ALL TABLES IN SCHEMA \"public\" FROM \"${READ_USER}\";" -U ${USER_NAME}
	PGPASSWORD=${USER_PASSWORD} psql -c "REVOKE ALL ON ALL SEQUENCES IN SCHEMA \"public\" FROM \"${READ_USER}\";" -U ${USER_NAME}
	PGPASSWORD=${USER_PASSWORD} psql -c "REVOKE ALL ON ALL FUNCTIONS IN SCHEMA \"public\" FROM \"${READ_USER}\";" -U ${USER_NAME}
	PGPASSWORD=${POSTGRES_PASSWORD} psql -c "DROP USER IF EXISTS \"${READ_USER}\";" -U ${POSTGRES_USER}
	PGPASSWORD=${POSTGRES_PASSWORD} psql -c "DROP ROLE IF EXISTS \"${READ_USER}\";" -U ${POSTGRES_USER}
	PGPASSWORD=${POSTGRES_PASSWORD} psql -c "CREATE USER \"${READ_USER}\" WITH NOSUPERUSER NOCREATEDB NOCREATEROLE INHERIT LOGIN NOREPLICATION;" -U ${POSTGRES_PASSWORD}
	PGPASSWORD=${USER_PASSWORD} psql -c "GRANT CONNECT ON DATABASE \"${DATABASE_NAME}\" TO \"${READ_USER}\";" -U ${USER_NAME}
	PGPASSWORD=${USER_PASSWORD} psql -c "GRANT USAGE ON SCHEMA \"public\" TO \"${READ_USER}\";" -U ${USER_NAME}
	PGPASSWORD=${USER_PASSWORD} psql -c "GRANT SELECT ON ALL TABLES IN SCHEMA \"public\" TO \"${READ_USER}\";" -U ${USER_NAME}
	PGPASSWORD=${USER_PASSWORD} psql -c "GRANT SELECT ON ALL SEQUENCES IN SCHEMA \"public\" TO \"${READ_USER}\";" -U ${USER_NAME}
	PGPASSWORD=${USER_PASSWORD} psql -c "GRANT EXECUTE ON ALL FUNCTIONS IN SCHEMA \"public\" TO \"${READ_USER}\";" -U ${USER_NAME}
...
do
	...
	PGPASSWORD=${POSTGRES_PASSWORD} psql -c "ALTER DEFAULT PRIVILEGES IN SCHEMA \"public\" REVOKE ALL ON TABLES FROM \"${WRITE_USER}\";" -U ${POSTGRES_USER}
	PGPASSWORD=${POSTGRES_PASSWORD} psql -c "ALTER DEFAULT PRIVILEGES IN SCHEMA \"public\" REVOKE ALL ON SEQUENCES FROM \"${WRITE_USER}\";" -U ${POSTGRES_USER}
	PGPASSWORD=${POSTGRES_PASSWORD} psql -c "ALTER DEFAULT PRIVILEGES IN SCHEMA \"public\" REVOKE ALL ON FUNCTIONS FROM \"${WRITE_USER}\";" -U ${POSTGRES_USER}
	PGPASSWORD=${POSTGRES_PASSWORD} psql -c "REVOKE ALL ON SCHEMA \"public\" FROM \"${WRITE_USER}\";" -U ${POSTGRES_USER}
	PGPASSWORD=${POSTGRES_PASSWORD} psql -c "REVOKE ALL ON DATABASE \"${DATABASE_NAME}\" FROM \"${WRITE_USER}\";" -U ${POSTGRES_USER}
	PGPASSWORD=${USER_PASSWORD} psql -c "REVOKE ALL ON SCHEMA \"public\" FROM \"${WRITE_USER}\";" -U ${USER_NAME}
	PGPASSWORD=${USER_PASSWORD} psql -c "REVOKE ALL ON DATABASE \"${DATABASE_NAME}\" FROM \"${WRITE_USER}\";" -U ${USER_NAME}
	PGPASSWORD=${USER_PASSWORD} psql -c "REVOKE ALL ON ALL TABLES IN SCHEMA \"public\" FROM \"${WRITE_USER}\";" -U ${USER_NAME}
	PGPASSWORD=${USER_PASSWORD} psql -c "REVOKE ALL ON ALL SEQUENCES IN SCHEMA \"public\" FROM \"${WRITE_USER}\";" -U ${USER_NAME}
	PGPASSWORD=${USER_PASSWORD} psql -c "REVOKE ALL ON ALL FUNCTIONS IN SCHEMA \"public\" FROM \"${WRITE_USER}\";" -U ${USER_NAME}
	PGPASSWORD=${POSTGRES_PASSWORD} psql -c "DROP USER IF EXISTS \"${WRITE_USER}\";" -U ${POSTGRES_USER}
	PGPASSWORD=${POSTGRES_PASSWORD} psql -c "DROP ROLE IF EXISTS \"${WRITE_USER}\";" -U ${POSTGRES_USER}
	PGPASSWORD=${POSTGRES_PASSWORD} psql -c "CREATE USER \"${WRITE_USER}\" WITH NOSUPERUSER NOCREATEDB NOCREATEROLE INHERIT LOGIN NOREPLICATION;" -U ${POSTGRES_PASSWORD}
	PGPASSWORD=${USER_PASSWORD} psql -c "GRANT CONNECT ON DATABASE \"${DATABASE_NAME}\" TO \"${WRITE_USER}\";" -U ${USER_NAME}
	PGPASSWORD=${USER_PASSWORD} psql -c "GRANT USAGE ON SCHEMA \"public\" TO \"${WRITE_USER}\";" -U ${USER_NAME}
	PGPASSWORD=${USER_PASSWORD} psql -c "GRANT ALL ON ALL TABLES IN SCHEMA \"public\" TO \"${WRITE_USER}\";" -U ${USER_NAME}
	PGPASSWORD=${USER_PASSWORD} psql -c "GRANT ALL ON ALL SEQUENCES IN SCHEMA \"public\" TO \"${WRITE_USER}\";" -U ${USER_NAME}
	PGPASSWORD=${USER_PASSWORD} psql -c "GRANT EXECUTE ON ALL FUNCTIONS IN SCHEMA \"public\" TO \"${WRITE_USER}\";" -U ${USER_NAME}
