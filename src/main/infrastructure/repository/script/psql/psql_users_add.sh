...
do
	...
	do
		...
		do
			...
			PGPASSWORD=${POSTGRES_PASSWORD} psql -c "CREATE USER \"${CURRENT_USER}\" WITH NOSUPERUSER NOCREATEDB NOCREATEROLE INHERIT LOGIN NOREPLICATION;" -U ${POSTGRES_USER} || true
			${DEBUG} && echo "Configuring permissions for user ${CURRENT_USER}"
			PGPASSWORD=${POSTGRES_PASSWORD} psql -c "GRANT ${USER_GROUP_PERMISSION_GRANT:-CONNECT, TEMPORARY} ON DATABASE \"${POSTGRES_DEFAULT_DATABASE}\" TO \"${CURRENT_USER}\";" -U ${POSTGRES_USER} ${POSTGRES_DEFAULT_DATABASE}
			${DEBUG} && echo "GRANT ${USER_GROUP_PERMISSION_GRANT:-CONNECT, TEMPORARY} ON DATABASE \"${POSTGRES_DEFAULT_DATABASE}\" TO \"${CURRENT_USER}\";"
			PGPASSWORD=${POSTGRES_PASSWORD} psql -c "GRANT ${USER_GROUP_PERMISSION_GRANT:-USAGE} ON SCHEMA \"${USER_GROUP_SCHEMA}\" TO \"${CURRENT_USER}\";" -U ${POSTGRES_USER} ${POSTGRES_DEFAULT_DATABASE}
			${DEBUG} && echo "GRANT ${USER_GROUP_PERMISSION_GRANT:-USAGE} ON SCHEMA \"${USER_GROUP_SCHEMA}\" TO \"${CURRENT_USER}\";"
			PGPASSWORD=${POSTGRES_PASSWORD} psql -c "GRANT ${USER_GROUP_PERMISSION_GRANT:-SELECT} ON ${USER_GROUP_TABLE:-ALL TABLES} IN SCHEMA \"${USER_GROUP_SCHEMA}\" TO \"${CURRENT_USER}\";" -U ${POSTGRES_USER} ${POSTGRES_DEFAULT_DATABASE}
			...
			PGPASSWORD=${POSTGRES_PASSWORD} psql -c "GRANT ${USER_GROUP_PERMISSION_GRANT:-SELECT} ON ALL SEQUENCES IN SCHEMA \"${USER_GROUP_SCHEMA}\" TO \"${CURRENT_USER}\";" -U ${POSTGRES_USER} ${POSTGRES_DEFAULT_DATABASE}
			${DEBUG} && echo "GRANT ${USER_GROUP_PERMISSION_GRANT:-SELECT} ON ALL SEQUENCES IN SCHEMA \"${USER_GROUP_SCHEMA}\" TO \"${CURRENT_USER}\";"
			PGPASSWORD=${POSTGRES_PASSWORD} psql -c "GRANT ${USER_GROUP_PERMISSION_GRANT:-EXECUTE} ON ALL FUNCTIONS IN SCHEMA \"${USER_GROUP_SCHEMA}\" TO \"${CURRENT_USER}\";" -U ${POSTGRES_USER} ${POSTGRES_DEFAULT_DATABASE}
