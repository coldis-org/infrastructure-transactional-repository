...
do
	...
	if [ "${CURRENT_USER}" != "${POSTGRES_PASSWORD}" ] && [ "${CURRENT_USER}" != "${POSTGRES_ADMIN_PASSWORD}" ]
	then
	
		# For each schema.
		for SCHEMA in $(PGPASSWORD=${POSTGRES_PASSWORD} psql -c "SELECT schema_name FROM information_schema.schemata;" -U ${POSTGRES_USER} --quiet)
		do
		
			# Removing user.
			${DEBUG} && echo "Removing user ${CURRENT_USER} permissions"
			PGPASSWORD=${POSTGRES_PASSWORD} psql -c "REVOKE ALL ON DATABASE \"${DATABASE_NAME}\" FROM \"${CURRENT_USER}\";" -U ${POSTGRES_USER} || true
