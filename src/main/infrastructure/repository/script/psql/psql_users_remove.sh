...
do
	...
	if [ "${CURRENT_USER}" != "${POSTGRES_ADMIN_PASSWORD}" ] && [ "${CURRENT_USER}" != "${POSTGRES_ADMIN_PASSWORD}" ]
	then
	
		# For each schema.
		for SCHEMA in $(PGPASSWORD=${POSTGRES_ADMIN_PASSWORD} psql -c -t "SELECT schema_name FROM information_schema.schemata;" -U ${POSTGRES_ADMIN_USER} --quiet)
		do
